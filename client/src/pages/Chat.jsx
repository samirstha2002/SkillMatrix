...
function Chat() {
  ...
  // Update last message in sidebar when new message arrives
  useEffect(() => {
    const handler = (data) => {
      setMessages((prev) => {
      ...
      setUsers((prev) =>
        prev.map((c) => {
          ...
          const chatUserId = c.user._id;
          if (chatUserId === otherId || chatUserId === receiverId) {
  ...
  return (
    <div className="h-screen flex bg-[#efeae2] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {selectedUser ? (
          <>
            {/* Chat Header — clickable → public profile */}
            <div
              className="flex items-center gap-3 px-6 py-3 bg-white border-b border-gray-100 shadow-sm cursor-pointer hover:bg-gray-50 transition"
            ...
            >
              ...
              <div
                ...
                ${isOnline(selectedUser._id) ? 'text-emerald-600 bg-emerald-50' : 'text-gray-400 bg-gray-100'}"
              >
                {isOnline(selectedUser._id) ? (
                  <span className="text-emerald-600 bg-emerald-50 px-2 py-1 rounded-full text-xs font-medium">
                    ● Online
                  </span>
                ) : (
                  <span className="text-gray-400 bg-gray-100 px-2 py-1 rounded-full text-xs font-medium">
                    ● Offline
                  </span>
                )}
              </div>
                    ...
                    {msgs.map((msg, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div
                            className={`flex flex-col ${isMine ? "items-end" : "items-start"} max-w-[65%]`}
                          >
                            <div
                            ...
                            >
                              {msg.message}
                              <span
                                className={`text-[10px] ml-2 align-bottom inline-block ${isMine ? "text-gray-400" : "text-gray-400"}`}
                              >
